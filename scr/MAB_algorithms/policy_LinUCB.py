...
class LinUCB:
    """
    LinUCB: for both 'Disjoint LinUCB' and 'Tied LinUCB'
    ...
    def __init__(self,alpha,user_f, block_f,n_recom):
        ...
        self.a_max = 0 #store the temporal chosen arm
        self.x = None
    ...
    def recommend(self, user_feature):
        ...
        #in case of multi-recommendation is set
        r_list = []
    ...
    def policy_evaluator(self, views_f):
        ...
        for i in range(range_x):
            #idx = random.choice(range(len(u))) #randomly pop a record
            idx = 0 #pop a record in order

            userid = u.pop(idx)
            href = h.pop(idx)
            #get the associated user feature
            user_feature = user_f[user_f.userid == userid].iloc[:,1:].values[0] 
            #if sum(user_feature)!=0:
            recommends = self.recommend(user_feature)# select one arm by current strategy
            if href in recommends:
                #if recommended item matches the one generated by log policy - Replay
                reward = 1
            else:
                reward = 0
